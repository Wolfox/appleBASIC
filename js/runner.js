this.runner = (function() {
  ...
  run.run = function(tree) {
    ...
    runTree = function(index) {
      ...
      if(GoToIsLine) {
        GoToFLAG = findIndexOnTree(tree, GoToFLAG);
        GoToIsLine = false;
      }
  ...
  function findIndexOnTree(tree, lineSearch) {
    var returnVal = findIndexOnTreeAux(tree, lineSearch, 0);
  ...
  function findIndexOnTreeAux(tree, lineSearch, index) {
    if(!(index < tree.length)) {
      return undefined;
    }
    ...
    return findIndexOnTreeAux(tree, lineSearch, index+1);
  }
